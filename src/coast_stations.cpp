...
// Note: std::map only stores one value per key. Where the SDRangel source had
// duplicate MMSI keys mapping to different station names, only the last entry
// for each key is retained (matching std::map initializer-list behavior).
const std::map<std::string, std::string> CoastStations {
