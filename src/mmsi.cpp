...
const std::map<int, std::string> MMSI::m_mid = {
    {201, "albania"},
    {202, "andorra"},
    {203, "austria"},
    {204, "portugal"},
...
std::string MMSI::getMID(const std::string &mmsi)
{
    if (mmsi.compare(0, 2, "00") == 0 || mmsi.compare(0, 2, "99") == 0 || mmsi.compare(0, 2, "98") == 0) {
        return mmsi.substr(2, 3);
    } else if (mmsi[0] == '0' || mmsi[0] == '8') {
        return mmsi.substr(1, 3);
    } else if (mmsi.compare(0, 3, "111") == 0) {
        return mmsi.substr(3, 3);
    } else {
        return mmsi.substr(0, 3);
    }
}

std::string MMSI::getCountry(const std::string &mmsi)
{
    std::string midStr = MMSI::getMID(mmsi);
    int mid = std::atoi(midStr.c_str());
    auto it = m_mid.find(mid);
    if (it != m_mid.end()) {
        return it->second;
    }
    return "";
...
std::string MMSI::getCategory(const std::string &mmsi)
{
    if (mmsi.empty()) {
        return "Unknown";
    }
    ...
    case '1':
        // Search and rescue
        if (mmsi.size() > 6 && mmsi[6] == '1') {
            return "SAR Aircraft";
        } else if (mmsi.size() > 6 && mmsi[6] == '5') {
    ...
    case '8':
        return "Handheld";
    case '9':
        if (mmsi.compare(0, 3, "970") == 0)
        {
            return "SAR";
        }
        else if (mmsi.compare(0, 3, "972") == 0)
        {
            return "Man overboard";
        }
        else if (mmsi.compare(0, 3, "974") == 0)
        {
            return "EPIRB"; // Emergency Beacon
        }
        else if (mmsi.compare(0, 3, "979") == 0)
        {
            return "AMRD"; // Autonomous
        }
        else if (mmsi.compare(0, 2, "98") == 0)
        {
            return "Craft with parent ship";
        }
        else if (mmsi.compare(0, 2, "99") == 0)
        {
            if (mmsi.size() > 5 && mmsi[5] == '1') {
                return "Physical AtoN";
            } else if (mmsi.size() > 5 && mmsi[5] == '6') {
                return "Virtual AtoN";
            } else if (mmsi.size() > 5 && mmsi[5] == '8') {
                return "Mobile AtoN";
            } else {
